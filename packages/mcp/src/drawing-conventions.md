...
- `zibel_doc_open` also takes SVG text, told apart by content: Zibel's own `zibel_export` SVG, a file saved in Inkscape, or plain SVG 1.1, at most 5 MB outside its embedded images (else `LIMIT_EXCEEDED`). Layers, pages, names, locks and `z-<id>` ids come back; units become pt, with px counting as pt. Clipping comes back as Clipping Masks, and embedded PNG, JPEG and GIF images as Images. What Zibel cannot hold yet (a clip it cannot hold, masks, filters, linked or WebP images and `<use>` are dropped) is listed once per kind in `warnings`; it never fails the open.
