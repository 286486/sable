...
**Replace（替换）**：
用一个文件更新已有的 Document：以导出时的 Revision 为基准三方合并，只应用文件相对基准改动的属性，作为一个可撤销的 Transaction。它是合并，不是覆盖。
_Avoid_: Overwrite、Reload、Sync

**Place（置入）**：
把一个 SVG 作为一个 Group 放进已有 Document 的某个父级，全部分配新 id，对应 Illustrator 的 File > Place 与粘贴。
